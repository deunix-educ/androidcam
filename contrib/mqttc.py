...
# mqtt service
#
import pathlib, json, time, logging, ssl
...
class MqttBase(object):
    def __init__(self, **p):
        ...
        if self.ca_cert:
            self.ca_cert = CUR_PATH / self.ca_cert 
    ...
    def client_set_subscriptions(self):
        ...
        logger.info(f"client_set_subscription {self.subscriptions}")
    ...
    def _publish_bytes(self, topic, payload, **conf):
        try:
            ...
            logger.error(f"_publish_bytes error: {e}")
    ...
    def _on_connect(self, client, userdata, flags, rc):
        msg = f"{client._client_id} connected, status {rc}, host={self.host}:{self.port}\, username={self.username}"
        try:
            if rc:
                raise
            ...
            logger.error(f"_on_connect error {e}")


    def _on_disconnect(self, client, userdata, rc):
        logger.info(f"Disconnected: {client._client_id} with status {rc}")
        try:
            j = 3
            for i in range(j):
    ...
    def on_receice_message_callback(self, topic, payload):
        return None
    ...
    def _on_message(self, client, userdata, message):
        try:
            if self.on_receice_message_callback(message.topic, message.payload):
                return
            try:
            ...
            logger.error(f"_on_message error {e}\n    {message.topic} {message.payload[:80]}")
